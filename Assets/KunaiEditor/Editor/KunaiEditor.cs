	...
	public class KunaiEditor : EditorWindow 
	{
			...
			if (obj2 != obj && obj != null)
			{
				...
				if(obj != null)
				{
					...
					if (obj is GameObject)
					{
						...
						particleColor = gObj.particleSystem.startColor;
						//If that GameObject is a ParticleSystem
						if (gObj.GetComponent<ParticleSystem>() != null)
						{
							notify = "Particle";
							particlesList.Add (gObj.gameObject);
							particleColors.Add (gObj.particleSystem.startColor);
							particleSizes.Add (gObj.particleSystem.startSize);
							particleSpeeds.Add (gObj.particleSystem.startSpeed);
							particleGravity.Add (gObj.particleSystem.gravityModifier);
						...
						foreach (GameObject temp in tempList)
						{
							if(temp != null)
							{
								...
								if (temp.GetComponent<ParticleSystem>() != null)
								{
									...
									particleColors.Add (temp.particleSystem.startColor);
									particleSizes.Add (temp.particleSystem.startSize);
									particleSpeeds.Add (temp.particleSystem.startSpeed);
									particleGravity.Add (temp.particleSystem.gravityModifier);
		...
		void GetChildren(GameObject parentObj)
		{
			...
			foreach (Transform child in parentObj.transform)
			{
				...
				if (child.GetComponent<ParticleSystem>() != null)
				{
					...
					particleColors.Add (child.particleSystem.startColor);
					particleSizes.Add (child.particleSystem.startSize);
					particleSpeeds.Add (child.particleSystem.startSpeed);
					particleGravity.Add (child.particleSystem.gravityModifier);
		...
		void ResetScales ()
		{
			...
			for(int i = 0; i < particlesList.Count; i++)
			{
				if(particlesList[i].GetComponent<ParticleSystem> () != null)
				{
					...
					particlesList[i].particleSystem.gravityModifier = particleGravity[j] * scaleFloat;
		...
		void SetColors ()
		{
			...
			for(int i = 0; i < particlesList.Count; i++)
			{
				if(particlesList[i].GetComponent<ParticleSystem> () != null)
				{
					particlesList[i].particleSystem.startColor = particleColor;
					j++;
				}
			}
		}
		...
		void ResetColors ()
		{
			...
			for(int i = 0; i < particlesList.Count; i++)
			{
				if(particlesList[i].GetComponent<ParticleSystem> () != null)
				{
					particlesList[i].particleSystem.startColor = particleColors[j];
					j++;
				}
			}
		}
